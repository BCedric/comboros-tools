...
const ArtistsForm = () => {
  ...
  useEffect(() => {
    if (accessCode != null) {
      Http.get(`/artist/${accessCode}`)
        .then((artist) =>
          setFormFields({
            ...artist,
            band: artist.band != null ? artist.band.id : null,
            dateArrival: moment(artist.dateArrival).format('YYYY-MM-DDTHH:mm'),
            dateDeparture: moment(artist.dateDeparture).format(
              'YYYY-MM-DDTHH:mm'
            )
          })
        )
  ...
  const submit = useCallback((obj) => {
    confirm(
      ...
      () => {
        if (accessCode != null) {
          return Http.put(`/artist/${accessCode}`, obj)
            .then(() => setIsEnded(true))
            .catch(() => setHasError(true))
        } else {
          return Http.post('/artist', obj)
            .then(() => setIsEnded(true))
            .catch(() => setHasError(true))
        }
      }
